...
def add_attendance_data(event, context, date_query=None):
    ...
    if AttendanceHandler.get_attendance(child_id=child_id) is None:
        child_attendance = AttendanceHandler.add_attendance(child_id=child_id, kindergarten_id=kindergarten_id)
    ...
    AttendanceHandler.update_attendance(child_id=child_attendance.get(CHILD_ID, None),
                                        date_query=child_attendance.get(DATE, None),
                                        kindergarten_id=child_attendance.get(KINDERGARTEN_ID, None),
                                        time_out=child_attendance.get(TIME_OUT, None),
                                        time_in=child_attendance.get(TIME_IN, None),
